...
def delete_user():
    ...
    os.system('ldapdelete - x -D "cn=admin,dc=dezmeana,dc=local,dc=au" \
    -w "uid=%s,ou=people,dc=dezmeana,dc=local,dc=au"' % usr)
...
def srch_usr():
    usr = input("Enter username: ")
    userinfo = os.system('ldapsearch -x -b "cn=admin,dc=dezmeana,dc=local,dc=au" \
    | grep -B 13 home/%s' % usr)
    print(userinfo)
    main_menu()
